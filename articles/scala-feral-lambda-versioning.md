...
1. version-propertyの設定（デフォルトはproject.version）を見て、sys.propsに値があるかどうかをチェックする。もしあれば、それを使う。
2. Gitのタグを調べます。gitTagToVersionNumber の設定に最初にマッチしたものが、バージョンの割り当てに使われます。デフォルトでは、v と数字で始まるタグを探し、その数字をバージョンとして使用します。複数のバージョンタグがある場合は、一番大きなものを選びます。
3. タグも見つからない場合は、先頭のコミットを参照し、これを git.baseVersion の設定にアタッチします。`<base-version>.<git commit sha>` となります。
...
sbtプロジェクトのバージョンも確認しておきましょう。以下のように指定した次のバージョンに更新されていて数値のみで構成されたバージョンとなっていることが確認できます。
...
スクリプトを書いてCI/CDで自動化することもできますが、sbt-gitを使用することで簡単にバージョン管理行いつつsbt-releaseとの併用もできますのでぜひ試してみてください。
